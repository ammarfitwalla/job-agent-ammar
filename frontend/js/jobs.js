import { getProfile, showToast, htmlEscape } from "./utils.js";
  ...
  el.innerHTML = jobs.map(j => {
    ...
    <div class="bg-white border border-slate-200 rounded-2xl p-4 sm:p-5 hover:border-indigo-300 hover:shadow-md transition-all group cursor-pointer flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
      <div class="flex items-start sm:items-center gap-4 w-full sm:w-auto">
        <div class="w-12 h-12 rounded-xl ${cc} flex items-center justify-center font-bold text-xl shrink-0">${_esc((j.company || "?").charAt(0).toUpperCase())}</div>
        <div class="flex-1 min-w-0">
          <h4 class="font-bold text-slate-900 text-base group-hover:text-indigo-600 transition-colors truncate">
            <a href="${_esc(j.url)}" target="_blank">${_esc(j.title || "Untitled Position")}</a>
          </h4>
          <div class="flex flex-wrap items-center text-sm text-slate-500 mt-0.5 gap-x-2 gap-y-1">
            <span class="font-medium text-slate-700">${_esc(j.company || "Unknown")}</span>
            <span class="hidden sm:inline">&bull;</span>
            ${j.location ? `<span class="truncate">${_esc(j.location)}</span>` : ""}
          </div>
          ${tags.length ? `<div class="flex flex-wrap items-center gap-1.5 mt-2">${tags.map(t => `<span class="text-xs font-medium bg-slate-100 text-slate-600 px-2 py-0.5 rounded-md">${_esc(typeof t === 'string' ? t : '')}</span>`).join("")}</div>` : ""}
        </div>
      </div>
      ...
      <div class="flex items-center justify-between w-full sm:w-auto gap-3 sm:pl-5 sm:border-l sm:border-slate-100 shrink-0">
        ...
        <button class="text-xs font-medium text-indigo-600 hover:text-indigo-700 bg-indigo-50 hover:bg-indigo-100 px-2.5 py-1.5 rounded-lg transition-colors referral-open-btn" data-job-title="${_esc(j.title || '')}" data-job-url="${_esc(j.url || '')}" data-company="${_esc(j.company || '')}" title="See referrals at this company">
...
document.addEventListener("click", (e) => {
  const refBtn = e.target.closest(".referral-open-btn");
  if (refBtn) {
    e.preventDefault();
    e.stopPropagation();
    ...
    window.showReferralUsers(refBtn.dataset.company);
    return;
  }
});
