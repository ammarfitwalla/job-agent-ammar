...
## Suggested build order (simple → complex)

1. **Housekeeping** — trivial decisions, no code (start here)
2. **#3 City in location search** — small backend + frontend
3. **#9 User location + admin registrations columns** — small-medium
4. **#6 Resume text at rest** — has a trivial variant; else medium
5. **#10 Security hardening** — medium, wide surface
6. **#8 Dedicated `/referrals` page** — large
7. **#11 Careers tab** — largest (new tables + scrapers + scheduler + admin tab)
8. **Load test** — gated on a go/no-go, not code

---

## Carry-over — Housekeeping (start here)
...
**Plan:** `docs/planning/city_location_plan.md` — new `/cities` endpoint from the offline `countrystatecity` package (no API key), free-text→city resolution in `_resolve_request_location` (scrape.py `_CITY_INDEX`), per-board token formatting, frontend city tier + `search.js` label handling. Tooling already built/verified: `test_cities_api.py` (PASS, 13k cities) + `fetch_cities_jobs.py` `--cities-from/--list-cities/--cities-file` flags.
...
## 6. Resume text at rest (browser-resident plaintext)

**What/why:** The resume is cached as plaintext in `localStorage` (plus a search-ID copy) and re-used to pre-fill referral/re-run flows. Server HTTPS transfer is fine — the concern is the browser-resident plaintext copy.
...
## Carry-over — Load test (last)

- `docs/planning/loadtest-plan-2026-09-07.md` written and approach agreed (k6 vs prod, unique X-Forwarded-For per VU) — not run yet; **k6 not installed locally**; needs a go/no-go before hammering prod.